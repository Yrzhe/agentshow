...
export async function renderWebhooksPage(root) {
  ...
  function latestLabel(hook) {
    ...
    return '<span style="color:' + (delivery.success ? '#3fb950' : '#f85149') + '">' + escapeHtml(delivery.event_type) + ' · ' + escapeHtml(String(delivery.status_code ?? 'ERR')) + '</span>';
  }
  ...
  function renderDeliveries(id) {
    ...
    return '<div class="panel" style="margin-top:12px"><h4 style="margin-bottom:12px">Recent deliveries</h4>' + deliveries.map((delivery) =>
      '<div style="padding:10px 0;border-top:1px solid #30363d">' +
      '<div style="display:flex;justify-content:space-between;gap:12px"><strong>' + escapeHtml(delivery.event_type) + '</strong><span style="color:' + (delivery.success ? '#3fb950' : '#f85149') + '">' + escapeHtml(String(delivery.status_code ?? 'ERR')) + '</span></div>' +
  ...
  function render() {
    ...
    root.innerHTML = '<section>' +
      ...
      (state.webhooks.length === 0 ? '<p class="muted">No webhooks configured yet.</p>' : state.webhooks.map((hook) =>
        '<div class="panel" style="margin-bottom:12px;padding:12px;background:#0d1117">' +
      ...
      '<label style="display:grid;gap:6px"><span class="muted">Name</span><input name="name" value="' + escapeHtml(state.form.name) + '" style="padding:8px 10px;background:#0d1117;border:1px solid #30363d;border-radius:6px;color:#e6edf3"></label>' +
      '<label style="display:grid;gap:6px"><span class="muted">URL</span><input name="url" value="' + escapeHtml(state.form.url) + '" placeholder="https://example.com/webhook" style="padding:8px 10px;background:#0d1117;border:1px solid #30363d;border-radius:6px;color:#e6edf3"></label>' +
      '<label style="display:grid;gap:6px"><span class="muted">Secret</span><input name="secret" value="' + escapeHtml(state.form.secret) + '" placeholder="optional" style="padding:8px 10px;background:#0d1117;border:1px solid #30363d;border-radius:6px;color:#e6edf3"></label>' +
