...
function renderTeamDetail(state) {
  ...
  return '<section><div class="page-header"><div><h2>' + escapeHtml(team.name) + '</h2><p class="muted">Team ' + escapeHtml(team.id) + ' · ' + escapeHtml(state.viewer_role) + '</p></div><a href="#/teams">Back to teams</a></div>' +
    ...
    '</div>' +
    (state.isOwner ? '<form id="team-rename-form" class="panel" style="display:flex;gap:8px;align-items:center;margin-bottom:16px"><input id="team-name-input" value="' + escapeHtml(team.name) + '" style="flex:1;min-width:180px;padding:0.8rem 1rem;border:1px dashed var(--border);background:var(--panel-alt);color:var(--text)"><button type="submit">Rename team</button><button type="button" id="team-delete-button">Delete team</button></form>' : '') +
    '<div class="split-layout">' +
      ...
      '<div style="display:grid;gap:16px">' +
        (state.can_manage ? '<form id="team-invite-form" class="panel" style="display:grid;gap:12px"><h3 style="margin:0">Invite Member</h3><input id="team-invite-email" type="email" placeholder="name@example.com" style="padding:0.8rem 1rem;border:1px dashed var(--border);background:var(--panel-alt);color:var(--text)"><button type="submit">Create invite</button></form>' : '') +
...
export async function renderTeamsPage(root) {
  ...
  try {
    ...
    root.innerHTML = '<section><div class="page-header"><div><h2>Teams</h2><p class="muted">Create a shared workspace for team activity and reporting.</p></div></div><form id="team-create-form" class="panel" style="display:flex;gap:8px;align-items:center;margin-bottom:16px"><input id="team-create-name" placeholder="New team name" style="flex:1;min-width:180px;padding:0.8rem 1rem;border:1px dashed var(--border);background:var(--panel-alt);color:var(--text)"><button type="submit">Create team</button></form>' + renderTeamCards(Array.isArray(payload.teams) ? payload.teams : []) + '</section>'
