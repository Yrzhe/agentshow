...
export const budgetRoutes = new Hono<AppType>()
budgetRoutes.use('*', flexAuth())

budgetRoutes.get('/', async (c) => {
  const userId = c.get('userId')
  const settings = await getBudgetSettings(c.env.DB, userId)
  const month = getMonthRange()
  const rows = await getTokensByDay(c.env.DB, userId, Math.max(1, month.days))
...
budgetRoutes.put('/', async (c) => {
  ...
  await upsertBudgetSetting(c.env.DB, c.get('userId'), budget_type, limit_usd, alert_threshold)
...
budgetRoutes.delete('/:type', async (c) => {
  ...
  await deleteBudgetSetting(c.env.DB, c.get('userId'), budgetType)
  return c.json({ status: 'ok' })
})
