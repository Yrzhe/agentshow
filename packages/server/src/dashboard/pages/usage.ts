...
export async function renderUsagePage(root) {
  ...
  const estimateCost = (input, output) => {
    const rate = { input: 3, output: 15 };
    return ((input / 1000000) * rate.input + (output / 1000000) * rate.output);
  };
  const totalInput = sessions.reduce((s, x) => s + Number(x.total_input_tokens ?? 0), 0);
  const totalOutput = sessions.reduce((s, x) => s + Number(x.total_output_tokens ?? 0), 0);
  const totalCost = estimateCost(totalInput, totalOutput);
  const formatCost = (v) => '$' + v.toFixed(2);
  ...
  root.innerHTML = '<section>' +
    ...
    '<div class="card-grid">' +
      ...
      '<div class="panel"><span class="muted">Estimated cost</span><h3>' + formatCost(totalCost) + '</h3></div>' +
      ...
      '<div class="panel"><h3>Top projects</h3>' +
        (topProjects.length === 0
          ...
          : '<div class="chart-list">' + topProjects.map(([slug, tokens]) => {
              const projCost = estimateCost(tokens * 0.1, tokens * 0.9);
              return '<div class="chart-row">' +
                '<div class="chart-row__label"><span>' + slug + '</span><strong>' + formatNumber(tokens) + ' · ~' + formatCost(projCost) + '</strong></div>' +
  ...
  try {
    ...
    if (dailyList.length === 0) {
      ...
      chartEl.innerHTML = dailyList.map((d) => {
        ...
        const dayCost = estimateCost(input, output);
        const pct = Math.max(4, Math.round(total / maxDay * 100));
        return '<div class="chart-row">' +
          '<div class="chart-row__label"><span>' + d.date + '</span><strong>' + formatNumber(total) + ' · ' + formatCost(dayCost) + '</strong></div>' +
