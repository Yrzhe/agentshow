...
export const costAttributionRoutes = new Hono<ServerAppType>()
costAttributionRoutes.use('*', flexAuth())

costAttributionRoutes.get('/by-project', async (c) => {
  const userId = c.get('userId')
  const days = Math.min(Number(c.req.query('days')) || 30, 365)
  const projects = getCostByProject(c.get('db'), userId, days).map((row) => ({
...
costAttributionRoutes.get('/by-session', async (c) => {
  ...
  const sessions = getCostBySession(c.get('db'), userId, projectSlug, days).map((row) => ({
...
costAttributionRoutes.get('/by-tool', async (c) => {
  const userId = c.get('userId')
  const days = Math.min(Number(c.req.query('days')) || 30, 365)
  const tools = getCostByToolType(c.get('db'), userId, days).map((row) => ({
