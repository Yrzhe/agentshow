...
export const sessionRoutes = new Hono<ServerAppType>()

sessionRoutes.use('*', flexAuth())

sessionRoutes.get('/', async (c) => {
  const sessions = getCloudSessions(c.get('db'), c.get('userId'), {
...
sessionRoutes.get('/:id', async (c) => {
  ...
  const session = getCloudSession(c.get('db'), c.get('userId'), sessionId)
  ...
  const events = getCloudEvents(c.get('db'), c.get('userId'), sessionId, { limit })
  return c.json({ session, events })
})

sessionRoutes.get('/:id/stats', async (c) => {
  const stats = getCloudSessionStats(c.get('db'), c.get('userId'), c.req.param('id'))
  return c.json(stats)
})
