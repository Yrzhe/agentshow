 ...
 *
 * Keyed by sharing domain and project id exactly as the handler keys it: a suite that seeded a
 * differently-named object would pass while proving nothing about the route.
 */
async function project(): Promise<{ store: Store; projectId: string }> {
...
async function file(store: Store, opts: {
  path: string;
  content: string;
  mimeType: string;
  visibility: ProjectFileVisibility;
